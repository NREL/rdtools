...
* Many kwargs have changed name (but not input order) to bring nomenclature into 
   ...
   * :py:func:`~rdtools.normalization.normalize_with_expected_power` input kwargs are now ``pv``, ``power_expected``, ``poa_global``.
   ...
   * :py:func:`~rdtools.soiling.soiling_srr` first four kwargs are now ``energy_normalized_daily``, ``insolation_daily``, ``reps``, ``precipitation_daily``.
