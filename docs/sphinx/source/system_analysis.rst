...
The class is centered around a dynamic "model graph" that allows the
...
The model graph inside a :py:class:`~.system_analysis.SystemAnalysis` object is
...
Here is a diagram of the default RdTools model graph.  The dependencies of
``sensor_degradation_results`` are bolded, showing how the graph traces
...
.. image:: _static/model_graph_diagram.png
   :alt: A diagram of the default RdTools model graph, highlighting the dependencies of sensor_degradation_results
...
The model graph recognizes that the ``calc_expected_power`` plugin
...
relationships in a structured way like this allows the model graph to
...
``cell_temperature`` beforehand -- the model graph will do it for you.
...
Here the model graph recognizes that this new plugin conflicts with the default
...
Second, entirely new models can be inserted into the model graph.
...
rest of the model graph and allow you to calculate it like anything else:
