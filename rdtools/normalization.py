...
def energy_from_power(power, target_frequency=None, max_timedelta=None, power_type='right_labeled'):
    ...
    interval from a power time series. NaN is filled where the
...
def _aggregate(time_series, target_frequency, max_timedelta=None, series_type='right_labeled'):
    '''
    ...
    a trapazoidal sum is used. For right labeled time series, a rectagular rule
    sum is used. If any interval in `time_series` is greater than `max_timedelta`,
    it is ommitted from the sum.
