...
def test_soiling_srr_with_precip(
    soiling_normalized_daily, soiling_insolation, soiling_times, clean_criterion, expected_sr
):
    ...
    np.random.seed(1977)
    sr, sr_ci, soiling_info = soiling_srr(
        soiling_normalized_daily, soiling_insolation, clean_criterion=clean_criterion, **kwargs
    )
    assert expected_sr == pytest.approx(
        sr, abs=1e-6
...
def test_soiling_srr_max_negative_slope_error(soiling_normalized_daily, soiling_insolation):
    ...
    with pytest.warns(UserWarning, match="20% or more of the daily data"):
        sr, sr_ci, soiling_info = soiling_srr(
            soiling_normalized_daily, soiling_insolation, reps=10, max_relative_slope_error=45.0
        )
