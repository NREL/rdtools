...
def test_soiling_srr(soiling_normalized_daily, soiling_insolation, soiling_times):
    ...
    sr, sr_ci, soiling_info = soiling_srr(
        soiling_normalized_daily, soiling_insolation, reps=reps
    )
    assert 0.964369 == pytest.approx(
        sr, abs=1e-6
    ...
    assert 0.960205 == pytest.approx(
        soiling_info["exceedance_level"], abs=1e-6
    ), "Exceedance level different from expected value"
    assert 0.984079 == pytest.approx(
        soiling_info["renormalizing_factor"], abs=1e-6
    ), "Renormalizing factor different from expected value"
    assert (
        len(soiling_info["stochastic_soiling_profiles"]) == reps
...
def test_soiling_srr_with_precip(
    soiling_normalized_daily,
    soiling_insolation,
    soiling_times,
    clean_criterion,
...
def test_soiling_srr_confidence_levels(soiling_normalized_daily, soiling_insolation):
    ...
    assert np.array([0.959322, 0.966066]) == pytest.approx(
        sr_ci, abs=1e-6
    ), "Confidence interval with confidence_level=95 different than expected"
    assert 0.962691 == pytest.approx(
        soiling_info["exceedance_level"], abs=1e-6
...
def test_soiling_srr_trim(soiling_normalized_daily, soiling_insolation):
    ...
    assert 0.978093 == pytest.approx(
        sr, abs=1e-6
    ), "Soiling ratio with trim=True different from expected value"
    assert (
        len(soiling_info["soiling_interval_summary"]) == 1
...
def test_soiling_srr_recenter_false(soiling_normalized_daily, soiling_insolation):
    ...
    assert (
        1 == soiling_info["renormalizing_factor"]
    ), "Renormalizing factor != 1 with recenter=False"
    assert 0.966387 == pytest.approx(
        sr, abs=1e-6
...
def test_soiling_srr_negative_step(soiling_normalized_daily, soiling_insolation):
    ...
    with pytest.warns(UserWarning, match="20% or more of the daily data"):
        sr, sr_ci, soiling_info = soiling_srr(
            stepped_daily, soiling_insolation, reps=10
        )
...
def test_soiling_srr_max_negative_slope_error(
    soiling_normalized_daily, soiling_insolation
):
    np.random.seed(1977)
    with pytest.warns(UserWarning, match="20% or more of the daily data"):
        sr, sr_ci, soiling_info = soiling_srr(
...
def test_soiling_srr_with_nan_interval(soiling_normalized_daily, soiling_insolation):
    ...
    with pytest.warns(UserWarning, match="20% or more of the daily data"):
        sr, sr_ci, soiling_info = soiling_srr(
            normalized_corrupt, soiling_insolation, reps=reps
        )
    assert 0.948792 == pytest.approx(
        sr, abs=1e-6
    ...
    with pytest.warns(UserWarning, match="20% or more of the daily data"):
        sr, sr_ci, soiling_info = soiling_srr(
            ...
            soiling_insolation,
            reps=reps,
            method="perfect_clean_complex",
            piecewise=True,
            neg_shift=True,
        )
    assert 0.974225 == pytest.approx(
        sr, abs=1e-6
    ), "Soiling ratio different from expected value when an entire interval was NaN"
...
def test_soiling_srr_outlier_factor(soiling_normalized_daily, soiling_insolation):
    ...
    assert (
        len(info["soiling_interval_summary"]) == 2
...
def test_soiling_srr_non_daily_inputs(test_param):
    ...
    """
    dummy_daily_explicit = pd.Series(
        0, index=pd.date_range("2019-01-01", periods=10, freq="d")
    )
    dummy_daily_implicit = pd.Series(
        0, index=pd.date_range("2019-01-01", periods=10, freq="d")
    )
...
def test_complex_sr_clean_threshold(
