...
@pytest.fixture()
def cods_normalized_daily(cods_times):
    N = len(cods_times)
    interval_1 = 1 - 0.005 * np.arange(0, 25, 1)
    interval_2 = 1 - 0.002 * np.arange(0, 25, 1)
    interval_3 = 1 - 0.001 * np.arange(0, 25, 1)
    ...
    cods_normalized_daily = pd.Series(data=repeated_profile[:N], index=cods_times)
    cods_normalized_daily = cods_normalized_daily * noise
